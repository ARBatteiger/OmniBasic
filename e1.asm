...
limit equ 25
; LN:6 x(3)=1
mov esi,x-(1*4); Get base for array
mov eax,3; LoadReg CONSTANT
shl eax,2; ArrayIndex*DataSize
...
; LN:7 for i=x(3) to limit
mov esi,x-(1*4); Get base for array
mov eax,3; LoadReg CONSTANT
shl eax,2; ArrayIndex*DataSize
add esi,eax; Array base+subscript
mov [_TmpVec1],esi; Set vector ptr (1)
mov edi,[_TmpVec1]; Load vector ptr
mov eax,[edi]; Get init value
mov [i],eax; Set init loop val
_Lbl1:
mov eax,[i]; Load LoopVar value
cmp eax,25; Compare LoopVar to ImmVal
jg _Lbl3; Finish loop if >
; LN:8 print i
mov eax, dword [i]; LoadReg
