...
CONST Reserved7Offs=152
CONST Reserved8Offs=156
...
TABLE ListBoxEventTable AS EventData
 "command",evCommandUsed,evCommandOffs,0
 "select",evSelectUsed,evSelectOffs,1
 "scroll",evScrollUsed,evScrollOffs,0
 "%",0,0,0
ENDTABLE
...
ENDIF
INC LabelNum
OutBuf="cmp [!"+ObjectNameSafe+"+"+STR$(ArrayOffs)+"],$ffffffff"
GOSUB OutCodeData
OutBuf="jne _Lbl"+STR$(LabelNum)
GOSUB OutCodeData
OutBuf="mov [!"+ObjectNameSafe+"],0"
GOSUB OutCodeData
...
OutBuf="invoke CreateWindowEx,"+TempExStyle+","+TempStr+",NULL,"+TempStyle+",0,0,"+DefaultWidth+","+DefaultHeight+",["+ObjectParent+"],"+STR$(ObjectID)+",[!hinstance],NULL"
GOSUB OutCodeData
OutBuf="mov ["+ObjectNameSafe+"],eax"
GOSUB OutCodeData
OutBuf="mov [!"+ObjectNameSafe+"+"+STR$(HandleOffs)+"],eax"
GOSUB OutCodeData
OutBuf="invoke SetWindowLong,["+ObjectNameSafe+"],GWL_USERDATA,!"+ObjectNameSafe
GOSUB OutCodeData

IF ObjectTypeSafe=4 THEN
	OutBuf="invoke GetComboBoxInfo,["+ObjectNameSafe+"],!cbSize"
	GOSUB OutCodeData
...
IF InFlag=TRUE THEN
	IF LAND(SymTable(SymNdx).DataMod,SubClassedBit)=0 THEN
		...
		OutBuf="invoke SetWindowLong,["+ObjectParent+"],GWL_WNDPROC,!SubClassProc"+STR$(ProcNum)
		GOSUB OutCodeData
...
CommaMain
GOSUB NexToken
OutBuf="invoke GetWindowLong,["+ObjectName+"],GWL_STYLE"
GOSUB OutCodeData
SELECT CASE TokenBuf
	CASE "true"
...
END SELECT
GOSUB OutCodeData
OutBuf="invoke SetWindowLong,["+ObjectName+"],GWL_STYLE,eax"
GOSUB OutCodeData
...
IF ErrNo<>0 THEN MainLoop
INC LabelNum
OutBuf="cmp [!"+ObjectName+"],$ffffffff"
GOSUB OutCodeData
OutBuf="je _Lbl"+STR$(LabelNum)
GOSUB OutCodeData
OutBuf="mov [!"+ObjectName+"],$ffffffff"
GOSUB OutCodeData
OutBuf="invoke DestroyWindow,["+ObjectName+"]"
GOSUB OutCodeData
OutBuf="_Lbl"+STR$(LabelNum)+":"
GOSUB OutCodeData
GOTO MainLoop
