...
CONST ExpTabSiz=300
CONST UserSym=193'  set to first user symbol in PreDefined
...
TABLE KeyTable
 ...
 0,"Vacant22",Vacant22X
 0,"Vacant23",Vacant23X
 0,"include",IncludeX
 0,"freeze",FreezeX
 0,"call",CallX
 0,"chdir",ChDirX
...
ELSE
	...
	ENDIF
ENDIF
OutBuf="invoke SetFilePointer,[_IOPthNum],"+TSymName+",0,FILE_BEGIN"
GOSUB OutCodeData
GOSUB SetStatus
GOSUB CallErr
GOTO MainLoop
...
DATA "bin$",FCN,0,68,0,0
DATA "oct$",FCN,0,69,0,0
...
IF PType=27 THEN
	...
	GOSUB OutCodeData
	IF TDataType=STRINGVAR THEN
		OutBuf="mov ["+SymTemp.SymName+"],"+STR$(TDetail)
		GOSUB OutCodeData
		OutBuf="mov ["+SymTemp.SymName+"],0"
		GOSUB OutCodeData
	ENDIF
	GOTO GrpCpxEnd
ENDIF
