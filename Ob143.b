* OmniBasic Copyright 1995,1996,1997,1998 Computer Design Lab
* All Rights Reserved
*
...
#SET Version=0
#SET risc=0
* Version: 0=Commercial, 1=Personal, 2=Demo (Shareware)
#SET WinType=0
...
#IF Version=1
 PRINT
 PRINT "Personal Version"
 PRINT
#ENDIF

#IF Version=2
 PRINT
 PRINT "*************** Shareware Version ***************"
 PRINT
 PRINT "To order call: (573) 236-4644"
...
#ENDIF
 ...
 PRINT "Copyright  1995, 1996, 1997, 1998 Computer Design Lab"
...
#ENDIF
 ...
 IF FcnMode=0 THEN
	...
	OutBuf="_Prompt='?';"
	GenMain
	IF ANSIFlag<>0 THEN\ REM Prevent unreferenced var warnings from SAS
		RegInit()\ REM main case
	ENDIF
    IF WinFlag=0 OR Machine<>6 THEN
...
#IF WinType=1
	IF FormFlag<>0 THEN
		...
		GetObjSym()
		ErrMain
		IF SymTemp.DataType<>OBJECT THEN
			ErrNo=ErrIVType
			GOTO MainLoop
		ENDIF
		REM Process Form Object
		SetObjNam=TokenSaf
		GetObjTyp()
		IF ObjectType="%" THEN
			ErrNo=ErrIVType
		...
		Remark()
		GenCode
		ON SymTemp.DataMod GOSUB FA1,FA2,FA3,FA4,FA5,FA6,FA7,FA8,FA9,FA10,FA11,FA12,FA13,FA14,FA15,FA16,FA17,FA18,FA19,FA20,FA21,FA22,FA23,FA24,FA25,FA26,FA27,FA28,FA29,FA30
		GOTO MainLoop
	ENDIF
#ENDIF\ REM WinType=1
...
* Add Pixmap Button
#IF WinType=1
 ObjectFcn="fl_add_pixmapbutton"
 GenObject()
* Get PixMap File
 CommaRet
 GetObjTxt()
 ErrRet
 OutBuf="fl_set_pixmapbutton_file("+SetObjNam+",_IOBuffer);"
 GenCode
#ENDIF\ REM WinType=1
 RETURN
...
* Add Bitmap Button
#IF WinType=1
 ObjectFcn="fl_add_bitmapbutton"
 GenObject()
* Get BitMap File
 CommaRet
 GetObjTxt()
 ErrRet
 OutBuf="fl_set_bitmapbutton_file("+SetObjNam+",_IOBuffer);"
 GenCode
#ENDIF\ REM WinType=1
 RETURN
...
* Add Round 3D Button
#IF WinType=1
 ObjectFcn="fl_add_round3dbutton"
 GenObject()
#ENDIF\ REM WinType=1
 RETURN
...
* Add Menu
#IF WinType=1
 ObjectFcn="fl_add_menu"
 GenObject()
* Add Menu items here
AddMenuItem
 NexToken()
 IF TokenLen=0 THEN
 	RETURN\ rem no more menu items
 ENDIF
 UnDoToken()
 CommaRet
 GetObjTxt()
 ErrRet
 OutBuf="fl_addto_menu("+SetObjNam+",_IOBuffer);"
 GenCode
 GOTO AddMenuItem
#ENDIF\ REM WinType=1
 RETURN
...
* Add BITMAP
#IF WinType=1
 ObjectFcn="fl_add_bitmap"
 GenObject()
* Get BitMap File
 CommaRet
 GetObjTxt()
 ErrRet
 OutBuf="fl_set_bitmap_file("+SetObjNam+",_IOBuffer);"
 GenCode
#ENDIF\ REM WinType=1
 RETURN
...
* Add PIXMAP
#IF WinType=1
 ObjectFcn="fl_add_pixmap"
 GenObject()
* Get PixMap File
 CommaRet
 GetObjTxt()
 ErrRet
 OutBuf="fl_set_pixmap_file("+SetObjNam+",_IOBuffer);"
 GenCode
#ENDIF\ REM WinType=1
 RETURN
...
* Add Choice
#IF WinType=1
 ObjectFcn="fl_add_choice"
 GenObject()
* Add Choice items here
AddChoiceItem
 NexToken()
 IF TokenLen=0 THEN
 	RETURN\ rem no more choice items
 ENDIF
 UnDoToken()
 CommaRet
 GetObjTxt()
 ErrRet
 OutBuf="fl_addto_choice("+SetObjNam+",_IOBuffer);"
 GenCode
 GOTO AddChoiceItem
#ENDIF\ REM WinType=1
 RETURN
...
* Add BROWSER
#IF WinType=1
 ObjectFcn="fl_add_browser"
 GenObject()
* Add Browser items here
AddBrowserItem
 NexToken()
 IF TokenLen=0 THEN
 	RETURN\ rem no more choice items
 ...
 GetObjTxt()
 ErrRet
 OutBuf="fl_addto_browser("+SetObjNam+",_IOBuffer);"
 GenCode
 GOTO AddBrowserItem
#ENDIF\ REM WinType=1
 RETURN
...
#IF WinType=1
 GetObjXY()
 ErrRet
 OutBuf="_ArgSafe1=_d0;"\ REM need this in case if text is array or cpx
 GenCode
 CommaRet
 GetObjTxt()
 ErrRet
 OutBuf=SetObjNam+"="+ObjectFcn+"("+ObjectType+",_ArgSafe0,_ArgSafe1,"+STR$(XSize)+","+STR$(YSize)+",_IOBuffer);"
 GenCode
#ENDIF\ REM WinType=1
 RETURN
		...
		ELSE
			WHILE TokenBuf<>")" DO
				IF ANSIArgCtr=10 THEN
					...
					GOTO MainLoop
				ENDIF
				IF TokenBuf="long" OR TokenBuf="int" OR TokenBuf="float" OR TokenBuf="real" OR TokenBuf="word" OR TokenBuf="short" OR TokenBuf="byte" OR TokenBuf="char" THEN
					INC ANSIArgCtr
				ELSE
					IF TokenBuf<>"," THEN
						ErrNo=ErrSyntax
...
FcnBuild
 ...
 IF NOT(BITTST(FcnSpec,1)) AND OutBuf<>"extern " THEN
    IF Machine<>5 AND ANSIFlag=0 THEN
...
FORMDx
 ...
 ENDIF
#IF WinType=1
 OutBuf=TokenSaf+"=fl_bgn_form(FL_UP_BOX,"+STR$(SymTable(SymNdx).DataAddr)+","+STR$(SymTable(SymNdx).DataSize)+");"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 IF AnyWinFlag=0 THEN
 	PRINT "FATAL ERROR.. FDESIGN Directive in non-window program"
 	PRINT "Use '-?' to get valid command line options"
 	END
 ENDIF
 StartFlag=$ff
...
FDesignLoop
 NexToken()
 IF TokenLen=0 THEN FDESIGNDx1
 UseFilNam=UseFilNam+TokenSaf
 GOTO FDesignLoop
FDESIGNDx1
 ON ERROR GOTO NoFDFile
 OPEN #FDFile,UseFilNam:READ
 ON ERROR
FDLoop
 GOSUB GetFDLine
 TempStr=LEFT$(SrcLine,1)
 IF TempStr="" THEN
 	TokenBuf=""
 	TokenSaf=""
 	GOTO FDLoop
 ENDIF
 IF TempStr="#" THEN FDEOF
 IF TempStr="-" OR TempStr="=" THEN
 	TokenBuf=LEFT$(SrcLine,28)
 ELSE
 	NexToken()
 ENDIF
 GOSUB FDProc
...
FDProc
 ...
 IF KeyNdx=0 THEN
 	RETURN
 ENDIF
 IF KeyNdx>3 THEN\ REM Pre-fetch first operand (except for delimiters)
 	NexToken()\ REM Eat ':'
 	NexToken()\ REM First operand (excpt for 'number' case
 ENDIF
 ON KeyNdx GOSUB FD1,FD2,FD3,FD4,FD5,FD6,FD7,FD8,FD9,FD10,FD11,FD12,FD13,FD14,FD15,FD16,FD17,FD18,FD19,FD20,FD21,FD22,FD23,FD24
 RETURN
...
* NUMBER
 TempStr=TokenBuf
 NexToken()\ REM Eat ':"
 NexToken()\ REM Get number
 IF TempStr="forms" THEN
 	FDNumForms=VAL(TokenBuf)
 ELSE
 	FDNumObjects=VAL(TokenBuf)
 ENDIF
 RETURN
...
* NAME
 IF TokenSaf<>"" THEN
	...
	IF SymFound=0 THEN
		ClrSym()
		IF fd_setop<>1 THEN
			SymTemp.DataType=OBJECT
			SymTemp.DataMod=fd_class
			SymTemp.Detail=fd_type|fd_objclass
			OutBuf="FL_OBJECT *"+fd_name+";"
 			GenVar			
		ELSE
			SymTemp.DataType=FORM
			SymTemp.DataMod=0
			SymTemp.Detail=0
			OutBuf="FL_FORM *"+fd_name+";"
 			GenVar
		ENDIF
		SymTemp.DataAddr=VAL(fd_width)
		SymTemp.DataSize=VAL(fd_height)			
		SymInsert()
		ErrRet
	ENDIF
 ELSE
...
* TYPE
 IF TokenBuf="0" THEN
 	TokenSaf="NORMAL_GROUP"
 ENDIF
 TokenBuf="FL_"+TokenSaf
 TokenSaf=TokenBuf
 fd_typestr=TokenSaf
 FDGetObjTyp()
 fd_objclass=TempObjClass
 fd_type=ObjTypNdx
 RETURN
...
FD15
* LCOL
 fd_labelcolor=TokenSaf
 RETURN
 
FD16
* LABEL
 UnDoToken()
 fd_labeltext=MID$(SrcLine,SrcNdx+1,LEN(SrcLine)-SrcNdx+1)
 RETURN
 
FD17
* SHORTCUT
 UnDoToken()
 fd_shortcut=MID$(SrcLine,SrcNdx+1,LEN(SrcLine)-SrcNdx+1)
 RETURN
...
FD19
* GRAVITY
 fd_gravity1=TokenSaf
 NexToken()
 fd_gravity2=TokenSaf
 RETURN
 
FD20
* CALLBACK
 fd_callback=TokenSaf
 RETURN
...
* FILE
 UnDoToken()
 fd_file=MID$(SrcLine,SrcNdx+1,LEN(SrcLine)-SrcNdx+1)
 RETURN
...
FDFormBgn
 OutBuf=fd_name+"=fl_bgn_form(FL_UP_BOX,"+fd_width+","+fd_height+");"
 GenCode
 RETURN
 
FDObject
 IF AddFcnNam="fl_bgn_group" THEN
 	GOSUB FDGroupBgn
 	RETURN
 ENDIF
 IF AddFcnNam="fl_end_group" THEN
 	GOSUB FDGroupEnd
 	RETURN
 ENDIF
 OutBuf=fd_name+"="+AddFcnNam+"("+fd_typestr+","+fd_xpos
 OutBuf=OutBuf+","+fd_ypos+","+fd_width+","+fd_height+","+CHR$($22)+fd_labeltext+CHR$($22)+");"
 GenCode
 IF fd_color1<>"" THEN
 	IF fd_color1<>"FL_COL1" OR fd_color2<>"FL_COL1" THEN
 		OutBuf="fl_set_object_color("+fd_name+","+fd_color1+","+fd_color2+");"
 		GenCode
 	ENDIF
 ENDIF
 IF fd_labeltext<>"" THEN
 	OutBuf="fl_set_object_label("+fd_name+","+CHR$($22)+fd_labeltext+CHR$($22)+");"
 	GenCode
 ENDIF
 IF fd_shortcut<>"" THEN
 	OutBuf="fl_set_object_shortcut("+fd_name+","+CHR$($22)+fd_shortcut+CHR$($22)+",1);"
 	GenCode
 ENDIF
 IF fd_callback<>"" THEN
 	SymTemp.SymName=fd_callback
 	SymSrch()
 	IF SymFound=0 THEN
		ClrSym()
		SymTemp.DataType=FCN
		SymTemp.Detail=$3f2
		OutBuf="void "+fd_callback+"();"
		GenVar
		SymTemp.DataMod=0
		SymInsert()
		ErrRet
 	...
 	IF SymTable(SymNdx).DataType<>FCN OR TempInt<>$3f2 THEN
		ErrNo=ErrTypMis
		RETURN
	ENDIF	
 	IF fd_objclass=OBJ_BROWSER THEN
 		OutBuf="fl_set_browser_dblclick_callback("+fd_name+","+fd_callback+","+STR$(fd_argument)+");"
 	ELSE
 		OutBuf="fl_set_object_callback("+fd_name+","+fd_callback+","+STR$(fd_argument)+");"
 	ENDIF
 	GenCode
 ENDIF
 IF fd_file<>"" THEN
 	IF fd_class=PIXMAPBUTTON THEN
 		OutBuf="fl_set_pixmapbutton_file("+fd_name+","+CHR$($22)+fd_file+CHR$($22)+");"
 		GenCode
 	ELSE
 		IF fd_class=BITMAPBUTTON THEN
 			OutBuf="fl_set_bitmapbutton_file("+fd_name+","+CHR$($22)+fd_file+CHR$($22)+");"
 			GenCode
 		ELSE
 			IF fd_class=PIXMAP THEN
 				OutBuf="fl_set_pixmap_file("+fd_name+","+CHR$($22)+fd_file+CHR$($22)+");"
 				GenCode
 			ELSE
 				IF fd_class=BITMAP THEN
 					OutBuf="fl_set_bitmap_file("+fd_name+","+CHR$($22)+fd_file+CHR$($22)+");"
 					GenCode
 				ENDIF
 			ENDIF
 		ENDIF
 ...
 ENDIF
 IF fd_return<>"" THEN
 	IF fd_class=DIAL THEN
 		OutBuf="fl_set_dial_return("+fd_name+","+fd_return+");"
 		GenCode
 	ELSE
 		IF fd_class=SLIDER THEN
 			OutBuf="fl_set_slider_return("+fd_name+","+fd_return+");"
 			GenCode
 		ELSE
 			IF fd_class=COUNTER THEN
 				OutBuf="fl_set_counter_return("+fd_name+","+fd_return+");"
 				GenCode
 			ELSE
 				IF fd_class=POSITIONER THEN
 					OutBuf="fl_set_positioner_return("+fd_name+","+fd_return+");"
 					GenCode
 				ELSE
 					IF fd_class=INPUT THEN
 						OutBuf="fl_set_input_return("+fd_name+","+fd_return+");"
 						GenCode
 					ENDIF
 				ENDIF
 			ENDIF
 	...
 	ENDIF
 ENDIF
 IF fd_style<>"" AND fd_style<>"FL_NORMAL_STYLE" THEN
 	OutBuf="fl_set_object_lstyle("+fd_name+","+fd_style+");"
 	GenCode
 ENDIF
 IF fd_alignment<>"" AND fd_alignment<>"FL_ALIGN_CENTER" THEN
 	OutBuf="fl_set_object_lalign("+fd_name+","+fd_alignment+");"
 	GenCode
 ENDIF 
 IF fd_resize<>"" AND fd_resize<>"FL_RESIZE_ALL" THEN
 	OutBuf="fl_set_object_resize("+fd_name+","+fd_resize+");"
 	GenCode
 ENDIF
 IF fd_gravity1<>"" THEN
 	IF fd_gravity1<>"FL_NoGravity" OR fd_gravity2<>"FL_NoGravity" THEN
 		OutBuf="fl_set_object_gravity("+fd_name+","+fd_gravity1+","+fd_gravity2+");"
 		GenCode
 	ENDIF
 ENDIF
 IF fd_labelcolor<>"" AND fd_labelcolor<>"FL_BLACK" THEN
 	OutBuf="fl_set_object_lcol("+fd_name+","+fd_labelcolor+");"
 	GenCode
 ENDIF
 IF fd_boxtype<>"" THEN
 	OutBuf="fl_set_object_boxtype("+fd_name+","+fd_boxtype+");"
 	GenCode
 ENDIF
 IF fd_size<>"" AND fd_size<>"FL_DEFAULT_SIZE" THEN
 	OutBuf="fl_set_object_lsize("+fd_name+","+fd_size+");"
 	GenCode
 ENDIF  
 RETURN
 
FDGroupBgn
 OutBuf=fd_name+"="+AddFcnNam+"();"
 GenCode
 RETURN
...
#IF WinType=1
 IF SymType="window" THEN
 	GenVar\ REM put out OniBasic source as comment
 	IF TDataType=FORM THEN
 		OutBuf="FL_FORM *"+SymTemp.SymName+";"
 	ENDIF
 	IF TDataType=OBJECT THEN
 		OutBuf="FL_OBJECT *"+SymTemp.SymName+";"
 	ENDIF
 	GenVar
 	GOTO MainLoop 
 ENDIF
...
#IF WinType=1
 GetObjFrmNam()
 IF ErrNo=0 THEN\ REM object or form
 	SetObjNam=TokenSaf
 	NexToken()
 	IF TokenBuf="gosub" THEN
 		...
 		FcnType=1
 	ELSE
 		IF TokenBuf="callback" THEN
 			FcnType=2
 		ELSE
 			IF TokenBuf="event" THEN
 				IF ObjForm="form" THEN
 					ErrNo=ErrIVType
 					GOTO MainLoop
 				ENDIF
 				FcnType=3
	...
	ENDIF
	IF FcnType<3 THEN 		
 		IF ObjForm="object" AND ObjClass=OBJ_BROWSER THEN
 			OutBuf="fl_set_browser_dblclick_callback("+SetObjNam+","+SymbolSaf+",0);"
 		ELSE
 			IF FcnType=2 AND ObjForm="object" THEN
 				CommaMain
 				GetIntOp()
 				ErrMain
 			ELSE
 				...
 				GenCode
 			ENDIF		
 			IF ObjForm="object" THEN
 				OutBuf="fl_set_object_callback("+SetObjNam+","+SymbolSaf+",_d0);"
 			ELSE
 				OutBuf="fl_set_form_callback("+SetObjNam+","+SymbolSaf+",0);"
 			ENDIF
 		ENDIF
 	ELSE
 		OutBuf="fl_set_object_posthandler("+SetObjNam+","+SymbolSaf+");"
 	ENDIF		
 	GenCode
 	GOTO MainLoop
 ENDIF
...
#IF WinType=1
 NexToken()
 IF TokenLen=0 OR TokenBuf="do" THEN
 	OutBuf="fl_do_forms();"
 ELSE
 	IF TokenBuf="check" THEN
  		OutBuf="f1_check_forms();"
 	ELSE
 		ErrNo=ErrSyntax
 		GOTO MainLoop
 	ENDIF
...
#IF WinType=1
 NexToken()
 IF TokenBuf="msg_box" THEN
 	CommaMain
 	GetObjTxt()
 	ErrMain
 	OutBuf="fl_show_messages(_IOBuffer);"
 	GenCode
 	GOTO MainLoop
 ENDIF
 IF TokenBuf="alert_box" THEN
 	CommaMain
 	GetObjTxt()
 	ErrMain
 	OutBuf="fl_show_alert(_IOBuffer,"+CHR$($22)+CHR$($22)+","+CHR$($22)+CHR$($22)+",0);"
 	GenCode
 	GOTO MainLoop
 ENDIF
 IF TokenBuf="hint_box" THEN
 	CommaMain
 	GetObjTxt()
 	ErrMain
 	CommaMain
 	GetObjXY()
 	ErrMain
 	OutBuf="fl_show_oneliner(_IOBuffer,_ArgSafe0,_d0);"
 	GenCode
 	GOTO MainLoop
 ENDIF 
 SymTemp.SymName=TokenSaf
 ...
 IF SymFound=0 THEN
 	...
 	GOTO MainLoop
 ENDIF
 IF SymTable(SymNdx).DataType<>FORM AND SymTable(SymNdx).DataType<>OBJECT THEN
 	ErrNo=ErrIVType
 	GOTO MainLoop
 ENDIF
 IF SymTable(SymNdx).DataType=FORM THEN
 	Place=SymTable(SymNdx).Detail
 	SetObjNam=TokenSaf
 	CommaMain
 	GetObjTxt()
 	ErrMain
 	TempStr="x" 
 	IF Place=0 THEN
 		TempStr="FL_PLACE_SIZE"
 	ENDIF
 	IF Place=1 THEN
 		TempStr="FL_PLACE_POSITION"
 	ENDIF
 	IF Place=2 THEN
 		TempStr="FL_GEOMETRY"
 	ENDIF
 	IF Place=3 THEN
 		TempStr="FL_PLACE_ASPECT"
 	ENDIF
 	IF Place=4 THEN
 		TempStr="FL_PLACE_MOUSE"
 	ENDIF
 	IF Place=5 THEN
 		TempStr="FL_PLACE_CENTER"
 	ENDIF
 	IF Place=6 THEN
 		TempStr="FL_PLACE_FULLSCREEN"
 	ENDIF
 	IF Place=7 THEN
 		TempStr="FL_PLACE_FREE"
 	ENDIF
 	IF Place=8 THEN
 		TempStr="FL_PLACE_HOTSPOT"
 	ENDIF
 	IF Place=9 THEN
 		TempStr="FL_PLACE_CENTERFREE"
 	ENDIF
 	IF Place=10 THEN
 		TempStr="FL_PLACE_ICONIC"
 	ENDIF
 	IF TempStr="x" THEN\ REM invalid screen type
 		ErrNo=ErrIVType
 		GOTO MainLoop
	ENDIF 
 	OutBuf="fl_show_form("+SetObjNam+","+TempStr+",FL_FULLBORDER,_IOBuffer);"
 	GenCode
 ELSE
 	SetObjNam=TokenSaf
 	OutBuf="fl_show_object("+SetObjNam+");"
 	GenCode
 ENDIF
#ENDIF
 GOTO MainLoop
...
* SETWIN
 ...
 IF KeyNdx=0 THEN
 	...
 	GOTO MainLoop
 ENDIF
 ON KeyNdx GOTO SO1,SO2,SO3,SO4,SO5,SO6,SO7,SO8,SO9,SO10,SO11,SO12,SO13,SO14,SO15,SO16,SO17,SO18,SO19,SO20,SO21,SO22,SO23,SO24,SO25,SO26,SO27,SO28,SO29,SO30,SO31,SO32,SO33,SO34,SO35,SO36,SO37,SO38,SO39,SO40,SO41,SO42,SO43,SO44,SO45,SO46,SO47,SO48,SO49,SO50,SO51,SO52,SO53,SO54
 PRINT "FATAL ERORR... SETWIN Lookup Error"
 END
 
SO1
* COLOR
#IF WinType=1
 IF ObjForm<>"object" AND TempStr<>"hint_box" THEN
 	ErrNo=ErrIVType
 	GOTO MainLoop
 ENDIF
 EqualMain 
 GetObjXY()
 ErrMain
 IF SetObjNam<>"hint_box" THEN
 	OutBuf="fl_set_object_color("+SetObjNam+",_ArgSafe0,_d0);"
 ELSE
 	OutBuf="fl_set_oneliner_color(_ArgSafe0,_d0);"
 ENDIF
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
* SHORTCUT 
#IF WinType=1
 IF ObjClass<>OBJ_BUTTON AND ObjClass<>OBJ_MENU AND ObjClass<>OBJ_INPUT AND ObjClass<>OBJ_BROWSER THEN
 	ErrNo=ErrIVType
 	GOTO MainLoop
 ENDIF
 EqualMain
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_object_shortcut("+SetObjNam+",_IOBuffer,_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetObjXY()
 ErrMain
 OutBuf="fl_set_"+ObjForm+"_position("+SetObjNam+",_ArgSafe0,_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetObjXY()
 ErrMain
 OutBuf="fl_set_"+ObjForm+"_size("+SetObjNam+",_ArgSafe0,_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="_ArgSafe0=_d0;"\ REM need this in case if text is array or cpx
 GenCode
 CommaMain 	
 GetObjTxt()
 ErrMain 	
 OutBuf="fl_set_menu_item_shortcut("+SetObjNam+",_ArgSafe0,_IOBuffer);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetObjXY()
 ErrMain
 OutBuf="fl_set_input_cursorpos("+SetObjNam+",_ArgSafe0,_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 ErrMain
 IF ObjClass=OBJ_MENU THEN
 	OutBuf="fl_set_menu_item_mode("+SetObjNam+",_ArgSafe0,_d0);"
 ELSE
 	IF ObjClass=OBJ_CHOICE THEN
 		OutBuf="fl_set_choice_item_mode("+SetObjNam+",_ArgSafe0,_d0);"
 	ELSE
 		ErrNo=ErrIVType
 		GOTO MainLoop
 	ENDIF
...
#IF WinType=1
 ...
 GetFltOp()
 ErrMain
 OutBuf="_DblArgSafe0=_d0f;"
 GenCode
 CommaMain
 GetFltOp()
 ErrMain
 IF ObjClass=OBJ_CHART THEN
 	TempStr="chart"
 ELSE
 	IF ObjClass=OBJ_COUNTER THEN
 		TempStr="counter"
 	ELSE
 		IF ObjClass=OBJ_DIAL THEN
 			TempStr="dial"
 		ELSE
 			IF ObjClass=OBJ_SLIDER THEN
 				TempStr="slider"
 			ELSE
 				ErrNo=ErrIVType
 				GOTO MainLoop
 	...
 	ENDIF
 ENDIF
 OutBuf="fl_set_"+TempStr+"_bounds("+SetObjNam+",_DblArgSafe0,_d0f);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetObjTxt()
 ErrMain
 OutBuf="fl_set_object_label("+SetObjNam+",_IOBuffer);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetFltOp()
 ErrMain
 IF ObjClass=OBJ_POSITIONER THEN
 	TempStr="positioner"
 ELSE
 	IF ObjClass=OBJ_XYPLOT THEN
 		TempStr="xyplot"
 	ELSE
 		ErrNo=ErrIVType
 		GOTO MainLoop
 	ENDIF
 ENDIF
 OutBuf="fl_set_"+TempStr+"_xbounds("+SetObjNam+",_DblArgSafe0,_d0f);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetObjTxt()
 ErrMain
 OutBuf="fl_load_browser("+SetObjNam+",_IOBuffer);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
* BITMAP
#IF WinType=1
 IF ObjClass<>OBJ_BITMAP AND ObjClass<>OBJ_BUTTON THEN
 	ErrNo=ErrIVType
 	GOTO MainLoop
 ENDIF
 EqualMain 
 GetObjTxt()
 ErrMain
 IF ObjClass=OBJ_BITMAP THEN
 	OutBuf="fl_set_bitmap_file("+SetObjNam+",_IOBuffer);"
 ELSE
 	IF ObjType<>BITMAPBUTTON THEN
 		ErrNo=ErrIVType
 		GOTO MainLoop
 	ENDIF
 	OutBuf="fl_set_bitmapbutton_file("+SetObjNam+",_IOBuffer);"
...
* PIXMAP
#IF WinType=1
 IF ObjClass<>OBJ_PIXMAP AND ObjClass<>OBJ_BUTTON THEN
 	ErrNo=ErrIVType
 	GOTO MainLoop
 ENDIF
 EqualMain 
 GetObjTxt()
 ErrMain
 IF ObjClass=OBJ_PIXMAP THEN
 	OutBuf="fl_set_pixmap_file("+SetObjNam+",_IOBuffer);"
 ELSE
 	IF ObjType<>PIXMAPBUTTON THEN
 		ErrNo=ErrIVType
 		GOTO MainLoop
 	ENDIF
 	OutBuf="fl_set_pixmapbutton_file("+SetObjNam+",_IOBuffer);"
...
#IF WinType=1
 ...
 ErrMain
 IF ObjClass=OBJ_MENU THEN
 	OutBuf="fl_delete_menu_item("+SetObjNam+",_d0);"
 ELSE
 	IF ObjClass=OBJ_CHOICE THEN
 		OutBuf="fl_delete_choice("+SetObjNam+",_d0);"
 	ELSE
 		ErrNo=ErrIVType
 		GOTO MainLoop
 	ENDIF
...
#IF WinType=1
 ...
 ELSE
 	...
 	ENDIF
 ENDIF
 OutBuf="fl_set_"+TempStr+"_ybounds("+SetObjNam+",_DblArgSafe0,_d0f);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_delete_browser_line("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_select_browser_line("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_"+TempStr+"_return("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_deselect_browser_line("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 ELSE
 	 ...
 	 ENDIF
 ENDIF
 OutBuf="fl_clear_"+TempStr+"("+SetObjNam+");"	
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_xyplot_xgrid("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
* COUNTUP
#IF WinType=1
 IF ObjClass<>OBJ_TIMER THEN
 	ErrNo=ErrIVType
 	GOTO MainLoop
 ENDIF 
 OutBuf="fl_set_timer_countup("+SetObjNam+");"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 IF ObjClass<>OBJ_TIMER THEN
 	...
 	GOTO MainLoop
 ENDIF 
 OutBuf="fl_suspend_timer("+SetObjNam+");"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 IF ObjClass<>OBJ_TIMER THEN
 	...
 	GOTO MainLoop
 ENDIF 
 OutBuf="fl_resume_timer("+SetObjNam+");"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 ErrMain
 IF ObjClass=OBJ_MENU THEN
 	OutBuf="fl_replace_menu_item("+SetObjNam+",_ArgSafe0,_IOBuffer);"
 ELSE
 	IF ObjClass=OBJ_CHOICE THEN
 		OutBuf="fl_replace_choice("+SetObjNam+",_ArgSafe0,_IOBuffer);"
 	ELSE
 		ErrNo=ErrIVType
 		GOTO MainLoop
 	ENDIF
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_xyplot_ygrid("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetObjTxt()
 ErrMain
 OutBuf="fl_replace_browser_line("+SetObjNam+",_ArgSafe0,_IOBuffer);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetObjTxt()
 ErrMain
 OutBuf="fl_insert_browser_line("+SetObjNam+",_ArgSafe0,_IOBuffer);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetObjXY()
 ErrMain
 OutBuf="fl_set_input_color("+SetObjNam+",_ArgSafe0,_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_object_bw("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_button("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_object_lcol("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_object_lsize("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 ErrMain
 IF ObjClass=OBJ_SLIDER THEN
 	TempStr="slider_value"
 ELSE
 	IF ObjClass=OBJ_COUNTER THEN
 		TempStr="counter_value"
 	ELSE
 		IF ObjClass=OBJ_DIAL THEN
 			TempStr="dial_value"
 		ELSE
 	...
 	ENDIF
 ENDIF
 OutBuf="fl_set_"+TempStr+"("+SetObjNam+",_d0f);"	 	
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain 	
 OutBuf="fl_add_chart_value("+SetObjNam+",_DblArgSafe0,_IOBuffer,_d0);"	
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain 	
 OutBuf="fl_insert_chart_value("+SetObjNam+",_ArgSafe0,_DblArgSafe0,_IOBuffer,_d0);"	
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain 	
 OutBuf="fl_replace_chart_value("+SetObjNam+",_ArgSafe0,_DblArgSafe0,_IOBuffer,_d0);"	
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetFltOp()
 ErrMain 	
 OutBuf="fl_set_positioner_xvalue("+SetObjNam+",_d0f);"	
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetFltOp()
 ErrMain 	
 OutBuf="fl_set_positioner_yvalue("+SetObjNam+",_d0f);"	
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_object_dblclick("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 IF OpClass<>STRClass THEN
 	...
 	GOTO MainLoop
 ENDIF
 OutBuf="fl_set_input("+SetObjNam+",(char*)"+TSymName+"._Ptr);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_object_boxtype("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop 
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_object_lstyle("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_object_lalign("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_object_resize("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetObjXY()
 ErrMain
 OutBuf="fl_set_object_gravity("+SetObjNam+",_ArgSafe0,_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_object_shortcutkey("+SetObjNam+",_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetFltOp()
 ErrMain
 OutBuf="_FltArgSafe0=_d0f;"
 GenCode
 CommaMain
 GetFltOp()
 ErrMain
 OutBuf="_FltArgSafe1=_d0f;"
 GenCode
 CommaMain
 GetIntOp()
 ErrMain
 ...
 GetObjTxt()
 ErrMain
 OutBuf="_MovStr(&_CBufVec,&_IOBufVec);"
 GenCode
 CommaMain
 GetObjTxt()
 ErrMain
 OutBuf="_MovStr(&_CBuf1Vec,&_IOBufVec);"
 GenCode 
 CommaMain
 GetObjTxt()
 ErrMain
 OutBuf="fl_set_xyplot_data("+SetObjNam+",&_FltArgSafe0,&_FltArgSafe1,_ArgSafe0,_ConvBuf,_ConvBuf1,_IOBuffer);"
 GenCode  
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 ErrMain
 IF ObjClass=OBJ_SLIDER THEN
 	OutBuf="fl_set_slider_step("+SetObjNam+",_d0f);"
 	GenCode
 ELSE
 	IF ObjClass=OBJ_DIAL THEN
 		OutBuf="fl_set_dial_step("+SetObjNam+",_d0f);"
 		GenCode
 	ELSE
 		OutBuf="_DblArgSafe0=_d0f;"
 		GenCode
 		...
 		ErrMain
 		IF ObjClass=OBJ_COUNTER THEN
 			OutBuf="fl_set_counter_step("+SetObjNam+",_DblArgSafe0,_d0f);"
 			GenCode
 		ELSE
 			IF ObjClass=OBJ_POSITIONER THEN
 				OutBuf="fl_set_positioner_xstep("+SetObjNam+",_DblArgSafe0);"
 				GenCode
 				OutBuf="fl_set_positioner_ystep("+SetObjNam+",_d0f);"
 				GenCode
 			ELSE 			
 				ErrNo=ErrIVType
 				GOTO MainLoop
...
#IF WinType=1
 ...
 GetIntOp()
 ErrMain
 OutBuf="fl_set_cursor("+SetObjNam+"->window,_d0);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 EqualMain
 GetObjTxt()
 OutBuf="fl_set_form_title("+SetObjNam+",_IOBuffer);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 GetFrmNam()
 ErrMain
 OutBuf="fl_freeze_form("+SymTemp.SymName+");"
 GenCode
 GOTO MainLoop
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 GetFrmNam()
 ErrMain
 OutBuf="fl_unfreeze_form("+SymTemp.SymName+");"
 GenCode
 GOTO MainLoop
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 GetObjFrmNam()
 ErrMain
 OutBuf="fl_redraw_"+ObjForm+"("+SymTemp.SymName+");"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetObjFrmNam()
 ErrMain
 OutBuf="fl_hide_"+ObjForm+"("+TokenSaf+");"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 IF SymTable(SymNdx).DataType=FORM THEN
 	FormGroup=0
 	OutBuf="fl_addto_form("+TokenSaf+");"
 ELSE
 	IF SymTable(SymNdx).DataMod<>GROUP THEN
 		ErrNo=ErrIVType
 		GOTO MainLoop
 	ENDIF
 	FormGroup=1 	
 	OutBuf="fl_addto_group("+TokenSaf+");"
 ENDIF
 GenCode
 CommaMain
 NexToken()
...
#IF WinType=1
 ...
 GetObjNam()
 ErrMain
 OutBuf="fl_delete_object("+TokenSaf+");"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 GetObjFrmNam()
 ErrMain
 OutBuf="fl_activate_"+ObjForm+"("+TokenSaf+");"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 GetObjFrmNam()
 ErrMain
 OutBuf="fl_deactivate_"+ObjForm+"("+TokenSaf+");"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetObjNam()
 ErrMain
 OutBuf="fl_set_focus_object("+SetObjNam+","+TokenSaf+");"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 GetObjNam()
 ErrMain
 OutBuf="fl_trigger_object("+TokenSaf+");"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 NexToken()
 IF TokenBuf="root" THEN
 	OutBuf="fl_get_mouse(&MX,&MY,&KM);"
 ELSE
 	UnDoToken()
 	GetFrmNam()
 	ErrMain
 	OutBuf="fl_get_form_mouse("+TokenSaf+",&MX,&MY,&KM);"
 ENDIF
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
#IF WinType=1
 ...
 OutBuf="_FltArgSafe1=_d0f;"
 GenCode
 OutBuf="fl_scale_"+ObjForm+"("+SetObjNam+",_FltArgSafe0,_FltArgSafe1);"
 GenCode
#ENDIF\ REM WinType=1
 GOTO MainLoop
...
ObjTyp DATA "FL_NORMAL_BUTTON",OBJ_BUTTON\ REM 1
 DATA "FL_PUSH_BUTTON",OBJ_BUTTON\ REM 2
 DATA "FL_TOUCH_BUTTON",OBJ_BUTTON\ REM 3
 DATA "FL_RADIO_BUTTON",OBJ_BUTTON\ REM 4
 DATA "FL_MENU_BUTTON",OBJ_BUTTON\ REM 5
 DATA "FL_HIDDEN_BUTTON",OBJ_BUTTON\ REM 6
 DATA "FL_INOUT_BUTTON",OBJ_BUTTON\ REM 7
 DATA "FL_RETURN_BUTTON",OBJ_BUTTON\ REM 8
 DATA "FL_HIDDEN_RET_BUTTON",OBJ_BUTTON\ REM 9
 DATA "FL_NO_BOX",OBJ_BOX\ REM 10
 DATA "FL_UP_BOX",OBJ_BOX\ REM 11
 DATA "FL_DOWN_BOX",OBJ_BOX\ REM 12
 DATA "FL_BORDER_BOX",OBJ_BOX\ REM 13
 DATA "FL_SHADOW_BOX",OBJ_BOX\ REM 14
 DATA "FL_FRAME_BOX",OBJ_BOX\ REM 15
 DATA "FL_ROUNDED_BOX",OBJ_BOX\ REM 16
 DATA "FL_EMBOSSED_BOX",OBJ_BOX\ REM 17
 DATA "FL_FLAT_BOX",OBJ_BOX\ REM 18
 DATA "FL_RFLAT_BOX",OBJ_BOX\ REM 19
 DATA "FL_RSHADOW_BOX",OBJ_BOX\ REM 20
 DATA "FL_OVAL_BOX",OBJ_BOX\ REM 21
 DATA "FL_ROUNDED3D_UPBOX",OBJ_BOX\ REM 22
 DATA "FL_ROUNDED3D_DOWNBOX",OBJ_BOX\ REM 23
 DATA "FL_OVAL3D_UPBOX",OBJ_BOX\ REM 24
 DATA "FL_OVAL3D_DOWNBOX",OBJ_BOX\ REM 25
 DATA "FL_PUSH_MENU",OBJ_MENU\ REM 26
 DATA "FL_PULLDOWN_MENU",OBJ_MENU\ REM 27
 DATA "FL_TOUCH_MENU",OBJ_MENU\ REM 28
 DATA "FL_NORMAL_INPUT",OBJ_INPUT\ REM 29
 DATA "FL_FLOAT_INPUT",OBJ_INPUT\ REM 30
 DATA "FL_INT_INPUT",OBJ_INPUT\ REM 31
 DATA "FL_DATE_INPUT",OBJ_INPUT\ REM 32
 DATA "FL_MULTILINE_INPUT",OBJ_INPUT\ REM 33
 DATA "FL_SECRET_INPUT",OBJ_INPUT\ REM 34
 DATA "FL_HIDDEN_INPUT",OBJ_INPUT\ REM 35
 DATA "FL_NO_FRAME",OBJ_FRAME\ REM 36
 DATA "FL_UP_FRAME",OBJ_FRAME\ REM 37
 DATA "FL_DOWN_FRAME",OBJ_FRAME\ REM 38
 DATA "FL_BORDER_FRAME",OBJ_FRAME\ REM 39
 DATA "FL_ENGRAVED_FRAME",OBJ_FRAME\ REM 40
 DATA "FL_EMBOSSED_FRAME",OBJ_FRAME\ REM 41
 DATA "FL_ROUNDED_FRAME",OBJ_FRAME\ REM 42
 DATA "FL_OVAL_FRAME",OBJ_FRAME\ REM 43
 DATA "FL_ANALOG_CLOCK",OBJ_CLOCK\ REM 44
 DATA "FL_DIGITAL_CLOCK",OBJ_CLOCK\ REM 45
 DATA "FL_NORMAL_BITMAP",OBJ_BITMAP\ REM 46
 DATA "FL_NORMAL_PIXMAP",OBJ_PIXMAP\ REM 47
 DATA "FL_NORMAL_TEXT",OBJ_TEXT\ REM 48
 DATA "FL_NORMAL_CHOICE",OBJ_CHOICE\ REM 49
 DATA "FL_DROPLIST_CHOICE",OBJ_CHOICE\ REM 50
 DATA "FL_NORMAL_BROWSER",OBJ_BROWSER\ REM 51
 DATA "FL_SELECT_BROWSER",OBJ_BROWSER\ REM 52
 DATA "FL_MOLD_BROWSER",OBJ_BROWSER\ REM 53
 DATA "FL_MULTI_BROWSER",OBJ_BROWSER\ REM 54
 DATA "FL_VERT_SLIDER",OBJ_SLIDER\ REM 55
 DATA "FL_HOR_SLIDER",OBJ_SLIDER\ REM 56
 DATA "FL_VERT_FILL_SLIDER",OBJ_SLIDER\ REM 57
 DATA "FL_HOR_FILL_SLIDER",OBJ_SLIDER\ REM 58
 DATA "FL_VERT_NICE_SLIDER",OBJ_SLIDER\ REM 59
 DATA "FL_HOR_NICE_SLIDER",OBJ_SLIDER\ REM 60
 DATA "FL_VERT_BROWSER_SLIDER",OBJ_SLIDER\ REM 61
 DATA "FL_HOR_BROWSER_SLIDER",OBJ_SLIDER\ REM 62
 DATA "FL_NORMAL_COUNTER",OBJ_COUNTER\ REM 63
 DATA "FL_SIMPLE_COUNTER",OBJ_COUNTER\ REM 64
 DATA "FL_NORMAL_DIAL",OBJ_DIAL\ REM 65
 DATA "FL_LINE_DIAL",OBJ_DIAL\ REM 66
 DATA "FL_FILL_DIAL",OBJ_DIAL\ REM 67
 DATA "FL_NORMAL_TIMER",OBJ_TIMER\ REM 68
 DATA "FL_VALUE_TIMER",OBJ_TIMER\ REM 69
 DATA "FL_HIDDEN_TIMER",OBJ_TIMER\ REM 70
 DATA "FL_BAR_CHART",OBJ_CHART\ REM 71
 DATA "FL_HORBAR_CHART",OBJ_CHART\ REM 72
 DATA "FL_LINE_CHART",OBJ_CHART\ REM 73
 DATA "FL_FILLED_CHART",OBJ_CHART\ REM 74
 DATA "FL_SPIKE_CHART",OBJ_CHART\ REM 75
 DATA "FL_PIE_CHART",OBJ_CHART\ REM 76
 DATA "FL_SPECIALPIE_CHART",OBJ_CHART\ REM 77
 DATA "FL_NORMAL_POSITIONER",OBJ_POSITIONER\ REM 78
 DATA "FL_BEGIN_GROUP",OBJ_GROUP\ REM 79
 DATA "FL_END_GROUP",OBJ_GROUP\ REM 80
 DATA "FL_NORMAL_CANVAS",OBJ_CANVAS\ REM 81
 DATA "FL_SCROLLED_CANVAS",OBJ_CANVAS\ REM 82
 DATA "FL_NORMAL_XYPLOT",OBJ_XYPLOT\ REM 83
 DATA "FL_SQUARE_XYPLOT",OBJ_XYPLOT\ REM 84
 DATA "FL_CIRCLE_XYPLOT",OBJ_XYPLOT\ REM 85
 DATA "FL_FILLED_XYPLOT",OBJ_XYPLOT\ REM 86
 DATA "FL_POINTS_XYPLOT",OBJ_XYPLOT\ REM 87
 DATA "FL_DASHED_XYPLOT",OBJ_XYPLOT\ REM 88
 DATA "FL_IMPULSE_XYPLOT",OBJ_XYPLOT\ REM 89
 DATA "FL_ACTIVE_XYPLOT",OBJ_XYPLOT\ REM 90
 DATA "FL_EMPTY_XYPLOT",OBJ_XYPLOT\ REM 91
 DATA "%",0 

ObjCls DATA "FL_BUTTON","fl_add_button"\ REM 1
 DATA "FL_CHECKBUTTON","fl_add_checkbutton"\ REM 2
 DATA "FL_ROUNDBUTTON","fl_add_roundbutton"\ REM 3
 DATA "FL_PIXMAPBUTTON","fl_add_pixmapbutton"\ REM 4
 DATA "FL_BITMAPBUTTON","fl_add_bitmapbutton"\ REM 5
 DATA "FL_LIGHTBUTTON","fl_add_lightbutton"\ REM 6
 DATA "FL_ROUND3DBUTTON","fl_add_round3dbutton"\ REM 7
 DATA "FL_BOX","fl_add_box"\ REM 8
 DATA "FL_MENU","fl_add_menu"\ REM 9
 DATA "FL_INPUT","fl_add_input"\ REM 10
 DATA "FL_FRAME","fl_add_frame"\ REM 11
 DATA "FL_LABELFRAME","fl_add_labelframe"\ REM 12
 DATA "FL_CLOCK","fl_add_clock"\ REM 13
 DATA "FL_BITMAP","fl_add_bitmap"\ REM 14
 DATA "FL_PIXMAP","fl_add_pixmap"\ REM 15
 DATA "FL_TEXT","fl_add_text"\ REM 16
 DATA "FL_CHOICE","fl_add_choice"\ REM 17
 DATA "FL_BROWSER","fl_add_browser"\ REM 18
 DATA "FL_SLIDER","fl_add_slider"\ REM 19
 DATA "FL_VALSLIDER","fl_add_valslider"\ REM 20
 DATA "FL_COUNTER","fl_add_counter"\ REM 21
 DATA "FL_DIAL","fl_add_dial"\ REM 22
 DATA "FL_TIMER","fl_add_timer"\ REM 23
 DATA "FL_CHART","fl_add_chart"\ REM 24
 DATA "FL_POSITIONER","fl_add_positioner"\ REM 25
 DATA "FL_BEGIN_GROUP","fl_bgn_group"\ REM 26
 DATA "FL_END_GROUP","fl_end_group"\ REM 27
 DATA "FL_CANVAS","fl_add_canvas"\ REM 28
 DATA "FL_GLCANVAS","fl_add_glcanvas"\ REM 29
 DATA "FL_XYPLOT","fl_add_xyplot"\ REM 30
 DATA "%","%"
...
#ENDIF
#IF WinType=1
 PRINT "    -x        Generate X-Windows executable"
#ENDIF
...
DecVar
 TempStr=""
 IF ExtVarFlg=0 THEN
	IF GblVarFlg=0 AND ParmFlag=0 AND FcnVarFlg=0 AND ANSIFlag=0 THEN
		TempStr="static "
	ENDIF
 ELSE
	TempStr="extern "
...
GetExp3
 ...
 IF TokenType=TTypSym THEN
	...
	IF SymFound=0 THEN
		...
		ELSE
			IF ANSIFlag=0 THEN
				TempStr="static long "+SymTemp.SymName+";"
			ELSE
...
#IF WinType=1
 ClassOp()
 IF OpClass<>OBJClass OR ObjClass<>OBJ_MENU THEN
 	ErrNo=ErrIVType
 	RETURN
 ENDIF
 OutBuf="STATUS=fl_get_menu("+TSymName+");"
 GenCode 
 GetIntTmp()
 ErrRet
 OutBuf=SymTemp.SymName+"=STATUS;"
...
* GETFILE()
#IF WinType=1
 ClassOp()\ REM Get Title
 IF TDataType<>STRING OR DMArray=$ff THEN
	ErrNo=ErrTypMis
	RETURN
 ENDIF
 OutBuf="_MovStr(&_IOBufVec,&"+TSymName+");"\ rem get form title into _IOBuffer
 GenCode
 OutBuf="fl_set_fselector_title(_IOBuffer);"
 GenCode
 INC ExpNdx
 ClassOp()\ REM Get Dir
 IF TDataType<>STRING OR DMArray=$ff THEN
	...
	RETURN
 ENDIF
 OutBuf="_MovStr(&_IOBufVec,&"+TSymName+");"\ rem get dir name into _IOBuffer
 GenCode 
 INC ExpNdx
 ClassOp()\ REM Get Pattern
 IF TDataType<>STRING OR DMArray=$ff THEN
	ErrNo=ErrTypMis
	RETURN
 ENDIF
 OutBuf="_MovStr(&_CBufVec,&"+TSymName+");"\ rem get dir name into _ConvBuf
 GenCode 
 ReqTmpVec()
 ErrRet
 OutBuf="(const char*)"+SymTemp.SymName+"._Ptr=fl_show_fselector("+CHR$($22)+CHR$($22)+",_IOBuffer,_ConvBuf,"+CHR$($22)+CHR$($22)+");"
 GenCode
 SymTable(SymNdx).DataType=STRING
 OutBuf="if ((long)"+SymTemp.SymName+"._Ptr) {"
 GenCode
 OutBuf=SymTemp.SymName+"._Len=0x7fffffff;"
 GenCode
 OutBuf=SymTemp.SymName+"._Lnk=0; }"
 GenCode
 OutBuf="else {"
...
#IF WinType=1
 ...
 IF OpClass<>INTClass THEN
 	...
 	RETURN
 ENDIF
 OutBuf="STATUS=fl_show_colormap("+TSymName+");"
 GenCode 
 GetIntTmp()
 ErrRet
 OutBuf=SymTemp.SymName+"=STATUS;"
...
#IF WinType=1
 ClassOp()
 IF OpClass<>OBJClass OR ObjClass<>OBJ_INPUT THEN
 	ErrNo=ErrIVType
 	RETURN
 ENDIF
 ReqTmpVec()
 ErrRet
 OutBuf="(const char*)"+SymTemp.SymName+"._Ptr=fl_get_input("+TSymName+");"
 GenCode
 SymTable(SymNdx).DataType=STRING
 OutBuf="if ((long)"+SymTemp.SymName+"._Ptr) {"
 GenCode
...
#IF WinType=1
 ...
 IF OpClass<>STRClass THEN
 	...
 	RETURN
 ENDIF
 OutBuf="_MovStr(&_CBufVec,&"+TSymName+");"\ rem get msg into _ConvBuf
 GenCode
 INC ExpNdx
 ClassOp()
 IF OpClass<>STRClass THEN
 	...
 	RETURN
 ENDIF
 OutBuf="_MovStr(&_IOBufVec,&"+TSymName+");"\ rem get default text into _IOBuffer
 GenCode
 ReqTmpVec()
 ErrRet
 OutBuf="(const char*)"+SymTemp.SymName+"._Ptr=fl_show_input(_ConvBuf,_IOBuffer);"
 GenCode
 SymTable(SymNdx).DataType=STRING
 OutBuf="if ((long)"+SymTemp.SymName+"._Ptr) {"
 GenCode
...
#IF WinType=1
 ...
 IF OpClass<>STRClass THEN
 	...
 	RETURN
 ENDIF
 OutBuf="_MovStr(&_IOBufVec,&"+TSymName+");"\ rem get question into _IOBuffer
 GenCode
 INC ExpNdx
 ClassOp()
 IF OpClass<>INTClass THEN
 ...
 LoadReg()
 ErrRet
 OutBuf="STATUS=fl_show_question(_IOBuffer,_d0);"
 GenCode 
 GetIntTmp()
 ErrRet
 OutBuf=SymTemp.SymName+"=STATUS;"
...
#IF WinType=1
 ClassOp()
 IF OpClass<>OBJClass OR ObjClass<>OBJ_BUTTON THEN
 	ErrNo=ErrIVType
 	RETURN
 ENDIF
 OutBuf="STATUS=fl_get_button("+TSymName+");"
 GenCode 
 GetIntTmp()
 ErrRet
 OutBuf=SymTemp.SymName+"=STATUS;"
...
#IF WinType=1
 ClassOp()
 IF OpClass<>OBJClass OR ObjClass<>OBJ_CHOICE THEN
 	ErrNo=ErrIVType
 	RETURN
 ENDIF
 OutBuf="STATUS=fl_get_choice("+TSymName+");"
 GenCode 
 GetIntTmp()
 ErrRet
 OutBuf=SymTemp.SymName+"=STATUS;"
...
#IF WinType=1
 ClassOp()
 IF OpClass<>OBJClass OR ObjClass<>OBJ_BROWSER THEN
 	ErrNo=ErrIVType
 	RETURN
 ENDIF
 OutBuf="STATUS=fl_get_browser("+TSymName+");"
 GenCode 
 GetIntTmp()
 ErrRet
 OutBuf=SymTemp.SymName+"=STATUS;"
...
#IF WinType=1
 ...
 ELSE
 	...
 	ENDIF
 ENDIF
 OutBuf=SymTemp.SymName+"=fl_get_"+TempStr+"("+UsrFcnNam+");"	
 GenCode
 ExpTable(ExpNdx)=SymNdx
#ENDIF\ REM WinType=1
 RETURN
...
#IF WinType=1
 ClassOp()
 IF OpClass<>OBJClass AND ObjClass<>OBJ_POSITIONER THEN
 	ErrNo=ErrIVType
 	RETURN
 ENDIF
 SetObjNam=TSymName 
 ...
 TDataType=FLOAT
 GetFltTmp()
 OutBuf=SymTemp.SymName+"=fl_get_positioner_xvalue("+SetObjNam+");"
 GenCode
 ExpTable(ExpNdx)=SymNdx
#ENDIF\ REM WinType=1
 RETURN
...
#IF WinType=1
 ...
 TDataType=FLOAT
 GetFltTmp()
 OutBuf=SymTemp.SymName+"=fl_get_positioner_yvalue("+SetObjNam+");"
 GenCode
 ExpTable(ExpNdx)=SymNdx
#ENDIF\ REM WinType=1
 RETURN
...
#IF WinType=1
 ...
 GetTmp()
 ErrRet
 OutBuf=SymTemp.SymName+"="+TSymName+"->belowmouse;"
 GenCode 
 ExpTable(ExpNdx)=SymNdx
#ENDIF\rem WinType=1
 RETURN
...
#IF WinType=1
 ClassOp()
 IF OpClass<>OBJClass AND OpClass<>FRMClass THEN
 	ErrNo=ErrIVType
 	RETURN
 ENDIF 
 GetIntTmp()
 ErrRet
 OutBuf=SymTemp.SymName+"="+TSymName+"->x;"
 GenCode
 ExpTable(ExpNdx)=SymNdx
#ENDIF\ REM WinType=1
 RETURN
...
#IF WinType=1
 ...
 GetIntTmp()
 ErrRet
 OutBuf=SymTemp.SymName+"="+TSymName+"->y;"
 GenCode
 ExpTable(ExpNdx)=SymNdx
#ENDIF\ REM WinType=1
 RETURN
...
#IF WinType=1
 ...
 GetIntTmp()
 ErrRet
 OutBuf=SymTemp.SymName+"="+TSymName+"->w;"
 GenCode
 ExpTable(ExpNdx)=SymNdx
#ENDIF\ REM WinType=1
 RETURN
...
#IF WinType=1
 ...
 GetIntTmp()
 ErrRet
 OutBuf=SymTemp.SymName+"="+TSymName+"->h;"
 GenCode
 ExpTable(ExpNdx)=SymNdx
#ENDIF\ REM WinType=1
 RETURN
...
#IF WinType=1
 ...
 IF ErrNo=0 THEN
 	...
 	RETURN
 ENDIF
 IF ErrNo=ErrUDSym THEN
 	RETURN
 ENDIF
 ErrNo=0\ REM give new chance
 UnDoToken()
 GetFrmNam()
 IF ErrNo=0 THEN
 	ObjForm="form"
...
#IF WinType=1
 SymTemp=SymTable(SymNdx)
 IF SymNdx<UserSym AND SymTemp.SymName<>"obj" THEN
 	ErrNo=ErrUDSym
 	RETURN
 ENDIF
 XSize=SymTemp.DataAddr
 YSize=SymTemp.DataSize
 ObjClass=SymTemp.Detail&$ff00
 ObjSubClass=SymTemp.Detail&$ff
 ObjType=SymTemp.DataMod
#ENDIF\ REM WinType=1
 RETURN
...
ObjTypLp
 INC TempInt
 READ ObjectType,TempObjClass
 IF TempInt=ObjSubClass THEN
 	IF ObjClass<>TempObjClass THEN\ REM wrong class
 		ObjectType="%"\ REM force error
 	ENDIF
 	RETURN
 ENDIF
 IF ObjectType<>"%" THEN ObjTypLp
#ENDIF\ REM WinType=1
 RETURN
...
FDObjTypLp
 ...
 IF ObjectType="%" THEN
 	...
 	RETURN
 ENDIF
 IF ObjectType<>TokenSaf THEN FDObjTypLp
#ENDIF\ REM WinType=1
 RETURN
...
FDObjClsLp
 INC ObjClsNdx
 READ ObjectType,AddFcnNam
 IF ObjectType="%" THEN
 	ObjClsNdx=0
 	RETURN
 ENDIF
 IF ObjectType<>TokenSaf THEN FDObjClsLp
#ENDIF\ REM WinType=1
 RETURN
...
PreProc
 SrcNdx=0
 NexToken()
 IF TokenBuf="#" OR TokenBuf=";" OR TokenBuf="+" OR TokenBuf="*" OR TokenBuf="'" OR TokenBuf=CHR$($5c) OR TokenBuf="," OR TokenBuf="." OR TokenBuf="/" OR TokenBuf="$" OR NextByte="*" THEN
	SrcLine=TAIL$(SrcLine,SrcNdx)
	IF TokenBuf="$" THEN
		TokenBuf="#"
	ENDIF
	IF TokenBuf=CHR$($5c) OR TokenBuf="'" THEN
		TokenBuf="*"
	ENDIF
